...
import classes from './RippleGrid.module.scss';
...
export const RippleGrid: FC<Props> = ({
  enableRainbow = false,
  gridColor = '#ffffff',
  ...
  const mousePositionRef = React.useRef({ x: 0.5, y: 0.5 });
  const targetMouseRef = React.useRef({ x: 0.5, y: 0.5 });
  const mouseInfluenceRef = React.useRef(0);
  ...
  React.useEffect(() => {
    ...
    uniforms.lightMode.value = lightMode;
  }, [
    enableRainbow,
    gridColor,
    rippleIntensity,
