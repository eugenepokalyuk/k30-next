...
/** Способ подтверждения. Union, а не строка: список закрыт бэкендом, и
 *  опечатка в нём должна ломать сборку, а не активацию у покупателя. */
type TargetKind = TargetOptionDto['kind'];

/** Состояние шага «Аккаунт»: два запроса и то, что между ними.
 *
 *  Вынесено из компонента, потому что это две разные вещи, которые
 *  меняются по разным поводам. Разметка правится, когда меняется вид
 *  экрана; этот код — когда меняется порядок проверки: добавился
 *  поставщик, не умеющий проверять аккаунт, или понадобилось иначе
 *  трактовать отказ. Держать их в одном файле значит каждый раз читать
 *  двести строк ради правки в десять.
 *
 *  Инвариант, ради которого хук вообще существует: подтверждение всегда
 *  относится к тем данным, которые сейчас в поле. Любое изменение ввода
 *  сбрасывает результат проверки — иначе покупатель подтвердит один
 *  аккаунт, поправит строку и активирует на другой, а это та ошибка,
 *  которую после активации уже не отменить.
 */
...
export interface TargetStepState {
  /** Выбранный способ подтверждения. undefined — форма не настроена. */
  ...
  isUnchecked: boolean;
  /** Проверка позади: показываем панель подтверждения вместо кнопки. */
  isConfirmed: boolean;
