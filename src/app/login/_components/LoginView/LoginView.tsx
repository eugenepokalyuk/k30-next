...
/**
 * Вход: код на почту и подтверждение в телеграм-боте. Пароля у
 * покупателя нет вовсе.
 *
 * Оба способа показываются только когда бэкенд говорит, что их есть чем
 * обслужить (`auth/options`): без SMTP код входа печатается в журнал
 * сервера, без токена бота подтверждать вход некому.
 *
          ...
          <>
            ...
            {telegram.stage === 'needs_code' && (
              <Notice tone="info" title="Бот ждёт код из письма">
                Мы отправили код на указанную почту — пришлите его в чат
                боту. Без этого шага к кабинету можно было бы привязать
                чужой адрес.
              </Notice>
            )}
